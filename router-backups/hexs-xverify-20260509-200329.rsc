# 2026-05-09 20:03:30 by RouterOS 7.18.2
...
add action=reject chain=trust-verdict comment=\
    "XVERIFY: reject HTTPS without router DNS witness" dst-port=443 log=yes \
    log-prefix=XVERIFY-TCP443 protocol=tcp reject-with=tcp-reset
add action=drop chain=trust-verdict comment=\
    "XVERIFY: drop QUIC/UDP443 without router DNS witness" dst-port=443 log=\
    yes log-prefix=XVERIFY-UDP443 protocol=udp
...
add interval=10s name=trust-list-refresh on-event="/system script run populate\
