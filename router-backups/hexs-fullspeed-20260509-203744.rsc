# 2026-05-09 20:37:45 by RouterOS 7.18.2
...
add action=drop chain=forward comment="Block YouTube by IP outbound" \
    disabled=yes dst-address-list=block-youtube log=yes log-prefix=\
    "YT-IP-OUT "
add action=drop chain=forward comment="Block YouTube DPI" layer7-protocol=\
    YouTube-L7 log=yes log-prefix="YT-DPI "
add action=drop chain=forward comment="Block YouTube by IP inbound" disabled=\
    yes log=yes log-prefix="YT-IP-IN " src-address-list=block-youtube
add action=drop chain=forward comment="Block Instagram DPI" layer7-protocol=\
    Instagram-L7
add action=drop chain=forward comment="Block YouTube QUIC/HTTP3" disabled=yes \
...
add action=accept chain=forward comment=\
    ...
    192.168.88.0/24
add action=accept chain=forward comment=\
    "XVERIFY: allow QUIC after router DNS witness" connection-state=new \
    dst-address-list=dns-witnessed dst-port=443 protocol=udp src-address=\
    192.168.88.0/24
add action=accept chain=forward comment=\
    "XVERIFY: allow HTTPS after router DNS witness" connection-state=new \
    dst-address-list=dns-witnessed dst-port=443 protocol=tcp src-address=\
    192.168.88.0/24
add action=accept chain=forward comment=\
    "NORMAL-WEB: allow Fast/Netflix HTTPS before generic proxy-suspect kill" \
    dst-address-list=normal-web-allow dst-port=443 protocol=tcp src-address=\
    192.168.88.0/24
add action=accept chain=forward comment=\
    "FULLSPEED: allow new allowed HTTPS before suspect kill" \
    connection-state=new dst-address-list=fullspeed-allow dst-port=443 \
    protocol=tcp src-address=192.168.88.0/24
add action=accept chain=forward comment=\
    "FULLSPEED: allow new allowed QUIC before suspect kill" connection-state=\
    new dst-address-list=fullspeed-allow dst-port=443 protocol=udp \
    src-address=192.168.88.0/24
add action=reject chain=forward comment=\
    "ENF2: kill proxy-suspect-2 TCP443 immediately" connection-mark=\
    proxy-suspect-2 dst-port=443 log-prefix=ENF2-S2-KILL protocol=tcp \
    reject-with=tcp-reset src-address=192.168.88.0/24
add action=reject chain=forward comment=\
    "ENF2: kill proxy-suspect-3 TCP443 immediately" connection-mark=\
    proxy-suspect-3 dst-port=443 log-prefix=ENF2-S3-KILL protocol=tcp \
    reject-with=tcp-reset src-address=192.168.88.0/24
add action=reject chain=forward comment=\
...
add action=reject chain=forward comment=\
    ...
    tcp-reset src-address=192.168.88.0/24
add action=fasttrack-connection chain=forward comment=\
    "FULLSPEED: fasttrack allowed download/speedtest dst" connection-state=\
    established,related dst-address-list=fullspeed-allow hw-offload=yes
add action=accept chain=forward comment=\
    "FULLSPEED: accept allowed download/speedtest dst" connection-state=\
    established,related dst-address-list=fullspeed-allow
add action=accept chain=forward comment=\
    "defconf: accept established,related, untracked" connection-state=\
    established,related,untracked
...
add action=return chain=trust-verdict comment=\
    "FULLSPEED: trusted by explicit DNS allowlist" dst-address-list=\
    fullspeed-allow
add action=reject chain=trust-verdict comment=\
    "XVERIFY: reject HTTPS without router DNS witness" dst-port=443 \
    log-prefix=XVERIFY-TCP443 protocol=tcp reject-with=tcp-reset
add action=drop chain=trust-verdict comment=\
    "XVERIFY: drop QUIC/UDP443 without router DNS witness" dst-port=443 \
    log-prefix=XVERIFY-UDP443 protocol=udp
