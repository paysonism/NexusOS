# 2026-05-09 20:05:47 by RouterOS 7.18.2
...
add action=reject chain=forward comment=\
    ...
    proxy-suspect-2 disabled=yes dst-port=443 log=yes log-prefix=ENF2-S2-KILL \
    protocol=tcp reject-with=tcp-reset src-address=192.168.88.0/24
add action=reject chain=forward comment=\
    "ENF2: kill proxy-suspect-3 TCP443 immediately" connection-mark=\
    proxy-suspect-3 disabled=yes dst-port=443 log=yes log-prefix=ENF2-S3-KILL \
    protocol=tcp reject-with=tcp-reset src-address=192.168.88.0/24
add action=reject chain=forward comment=\
...
add action=reject chain=trust-verdict comment=\
    "XVERIFY: reject HTTPS without router DNS witness" disabled=yes dst-port=\
    443 log=yes log-prefix=XVERIFY-TCP443 protocol=tcp reject-with=tcp-reset
add action=drop chain=trust-verdict comment=\
    "XVERIFY: drop QUIC/UDP443 without router DNS witness" disabled=yes \
    dst-port=443 log=yes log-prefix=XVERIFY-UDP443 protocol=udp
