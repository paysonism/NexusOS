...
Assert-Text $BootMemory 'APP_DATA_ADDR\s+equ\s+0x1000000' 'APP_DATA_ADDR invariant moved.'
Assert-Text $BootMemory 'APP_SLOT_SIZE\s+equ\s+0x100000' 'APP_SLOT_SIZE invariant moved.'
Assert-Text $BootMemory 'L3_SYSCALL_STACK_ADDR\s+equ\s+0x1800000' 'L3 syscall stack arena moved.'
...
Assert-Text $Registry 'Current public syscall numbers are `0\.\.21`' 'Registry must document current syscall range.'
